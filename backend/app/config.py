...
class Settings(BaseSettings):
    ...
    # One device claims one student identity within this window, so a student
    # cannot answer for their friends by signing in as each of them in turn on
    # the same phone. It expires so a shared or replaced device recovers
    ...
    # front of a class, and the only remedy must not be a rebuild: set
    # REQUEST_SLOTS=0 in the volume's config file to switch the cap off
    ...
    canvas_token: str = ""
    # The course this deployment is running. Defaulted rather than literally
    # hardcoded so next term is a config change, not a rebuild — but it works
    # out of the box with no configuration at all.
    canvas_course_id: int = 63598
